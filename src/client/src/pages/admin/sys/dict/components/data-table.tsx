...
import { fetchSysDictDelete, fetchSysDictPage } from "@/api/sys/dict";
import { DataTablePagination } from "@/components/data-table-pagination";

interface DataTableProps {
  columns: ColumnDef<SysDictDetailResponseType, any>[];
}

export type DictEditContextType = {
  id: string;
  setId: ({ id }: { id: string }) => void;
};
export const DictEditContext = createContext<DictEditContextType | null>(null);
...
export function DataTable({ columns }: DataTableProps) {
  ...
  const { data } = useQuery<SysDictPageResponseType>({
    queryKey: [
      QUERY_KEY.SYS_DICT_PAGE,
      pagination.pageIndex,
      pagination.pageSize,
      columnFilters,
    ],
    queryFn: async () => {
      const response = await fetchSysDictPage({
  ...
  >({
    ...
    onSuccess: (res) => {
      ...
      // 刷新
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY.SYS_DICT_PAGE] });
    },
    onError: (error) => {
      toast({
  ...
  return (
    <DictEditContext.Provider value={{ id: editId, setId: setEditIdAction }}>
