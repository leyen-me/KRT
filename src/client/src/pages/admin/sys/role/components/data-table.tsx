...
import { fetchSysRoleDelete, fetchSysRolePage } from "@/api/sys/role";
import { DataTablePagination } from "@/components/data-table-pagination";

interface DataTableProps {
  columns: ColumnDef<SysRoleDetailResponseType, any>[];
}

export type RoleEditContextType = {
  id: string;
  setId: ({ id }: { id: string }) => void;
};
export const RoleEditContext = createContext<RoleEditContextType | null>(null);
...
export function DataTable({ columns }: DataTableProps) {
  ...
  const { data } = useQuery<SysRolePageResponseType>({
    queryKey: [
      QUERY_KEY.SYS_ROLE_PAGE,
      pagination.pageIndex,
      pagination.pageSize,
      columnFilters,
    ],
    queryFn: async () => {
      const response = await fetchSysRolePage({
  ...
  >({
    ...
    onSuccess: (res) => {
      ...
      // 刷新
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY.SYS_ROLE_PAGE] });
    },
    onError: (error) => {
      toast({
  ...
  return (
    <RoleEditContext.Provider value={{ id: editId, setId: setEditIdAction }}>
